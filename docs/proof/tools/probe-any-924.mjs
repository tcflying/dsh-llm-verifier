...
const ENGINE = process.env.PROBE_ENGINE || "G:/zcode-project/llm-verify/dsh-llm-verifier/minimax-code-plugin/.minimax-plugin/mcp-server.mjs";
const data = mkdtempSync(join(tmpdir(), "probe924-"));
// A real repository root, for the refusals whose cause is supposed to be a config or a bound. Every
// negative test below used to pass `repoPath: data` — a plain scratch dir — and the engine refuses that
// on its own merits (`--is-inside-work-tree`, and since wave 24b the repo-root equality), so those
// assertions were certifying "the engine rejects a non-repo" and nothing else.
const repo = mkdtempSync(join(tmpdir(), "probe924repo-"));
...
// A refusal is only evidence when it names its own cause. `isError` alone is satisfied by any earlier
// validation on the same call, so every security refusal below has to match the engine's own wording.
const refused = (m, why, label) => {
  const t = textOf(m);
  const isErr = m.result?.isError === true || !!m.error;
  check(label, isErr && why.test(t), `${isErr ? "refused" : "ACCEPTED, no refusal at all"}; wanted ${String(why)}: ${t}`);
};
...
// ------------------------------------------------- 5. model injection, both doors
{
  ...
  s.close();   // this block opens a second session below; an unclosed child outlives the probe's exit
  ...
  const run = await s2.rpc("tools/call", {
    ...
    params: undefined,
    arguments: { repoPath: repo, task: "prove the injection door is closed" },
  }, 500);
  const t = textOf(run);
...
// ------------------------------------------------- 7. kill switch must fail closed
{
  ...
  const blocked = await s.rpc("tools/call", {
    name: "verified_best_of", arguments: { repoPath: repo, task: "must not run while disabled" },
  });
  refused(blocked, /is disabled; enable via verifier_configure/, "E13 best_of refuses while disabled, naming the kill switch");
  ...
  const tornRun = await s3.rpc("tools/call", {
    name: "verified_best_of", arguments: { repoPath: repo, task: "must not run on a torn config" },
  }, 701);
  refused(tornRun, /config is unreadable|treated as disabled until/, "E15 best_of refuses on a torn config, naming the config as the cause");
  s3.close();
}
...
// ------------------------------------------------- 8. candidate bounds are enforced server-side
{
  ...
  for (const n of [0, 2000, "2", 2.5]) {
    const m = await s.rpc("tools/call", {
      name: "verified_best_of", arguments: { repoPath: repo, task: "bounds check", candidateCount: n },
    });
    refused(m, /candidateCount/, `E16 candidateCount ${JSON.stringify(n)} refused by the bounds fence, not by an earlier check`);
  }
  s.close();
}
...
for (const [what, p] of [["scratch", data], ["repo fixture", repo]]) {
  try { rmSync(p, { recursive: true, force: true, maxRetries: 40, retryDelay: 250 }); }
  catch (e) { console.log(`(${what} dir left behind: ${p} — ${e.code})`); }
}
